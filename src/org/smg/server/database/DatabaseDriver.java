...
public class DatabaseDriver {
	...
	private static List<String> playerPropertyBlackList = Arrays
			.asList(PlayerProperty.EMAIL.toString(),
	...
	/**
	 * save player's information to database (insert or update) If it is to
	 * insert a profile, playerId should not be given return
	 ...
	 * return "UPDATED_PLAYER" if updated success return
	 * "WRONG_ACCESS_SIGNATURE" if update fail due to accessSignature
	 * authentication fail. return "WRONG_PLAYER_ID" if update fail due to no
	 * such id found.
	 * 
	 * @param player
	 * @return
	...
	private static String updatePlayer(Player player) {
		...
		try {
			...
			String as = (String) playerDB
					.getProperty(PlayerProperty.ACCESSSIGNATURE.toString());
			String asLocal = playerInfo
					.containsKey(PlayerProperty.ACCESSSIGNATURE.toString()) ? ""
	...
	private static String insertPlayer(Player player) {
		...
		Filter emailFilter = new FilterPredicate(EMAIL,
				Query.FilterOperator.EQUAL, playerInfo.get(EMAIL));
		...
		synchronized (insertPlayerMutex) {
			...
			String accessSignature = AccessSignatureUtil
					.getAccessSignature(String.valueOf(playerId));
